...
`SNS_AUTH_REQUIRED login_required`가 출력되지 않고 수집이 정상 진행되면 갱신 성공입니다. X 갱신 검증은 아래 "수동 갱신 성공 기준" 섹션을 따로 따릅니다.

> 앱 UI에는 과거 "인증 갱신" 패널과 `renew.py --web` 흐름이 있었으나, 운영에서 동작하지 않아 호출이 끊긴 상태입니다. 인증 갱신은 위 PowerShell 명령(터미널)으로만 진행하세요. 죽은 코드 정리는 BACKLOG `BL-0505-03`에서 추적합니다.
...
X(Twitter) 인증은 `AUTH_HOME/x/user_data/`를 유일한 수동 로그인 정본으로 사용합니다. `AUTH_HOME/x/cookies.json`, `AUTH_HOME/x/storage_state.json`, `AUTH_HOME/x_cookies_current.json`은 `user_data`에서 파생된 산출물입니다.

- `twitter_scrap.py`: `AUTH_HOME/x/user_data/`로 북마크 목록 수집
- `twitter_scrap_single.py`: `AUTH_HOME/x/cookies.json`에서 `auth_token`, `ct0`를 읽어 `twitter-cli` 상세 조회 수행
- `inject_x_cookies.py`: current cookie snapshot을 persistent profile에 재주입
- 쿠키만 갱신된 상태는 X 인증 갱신 완료로 보지 않음
...
같은 `AUTH_HOME/x/user_data/`를 여는 producer probe와 smoke test는 동시에 실행하지 않습니다.
