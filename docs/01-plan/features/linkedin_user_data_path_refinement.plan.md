---
title: "linkedin_user_data_path_refinement 기획서"
...
- [ ] `full` 데이터 파일명 변경: `linkedin_py_full_{date}.json` -> `linkedin_{USER_ID}_full_{date}.json`
...
- **기존 데이터 호환성**: `get_latest_full_file`에서 새로운 패턴(`linkedin_{USER_ID}_full_*.json`)과 이전 패턴(`linkedin_py_full_*.json`)을 모두 검색할지, 아니면 깔끔하게 새 패턴만 지원할지 결정이 필요합니다. (사용자에게 질문 예정)
