...
⚠️ 이 어댑터는 노출수(`view_count`)만 쓴다. 반응·댓글은 비로그인 경로
   (`linkedin_metric_single.py`)가 채우므로 여기서 온 값으로 덮어쓰지 않는다.
   병합 규칙은 `merge_own_post()` 가 강제한다(계획 3.6).

   원래 이유는 「로그인 카드가 반응수를 상위 몇 건만 준다」였다(계획 1.1 #7·#8 실측:
   impressions 36/36, reactions 5/36). **2026-09-05 Buffer 전환으로 그 전제는
   사라졌다** - Buffer 는 노출수·반응수·댓글수를 전건 준다(실측 37/37/37이고
   반응수는 비로그인 경로 값과 완전 일치). 그런데도 분담을 그대로 두는 것은
   이번 작업이 **복구**이기 때문이다. 경로를 하나로 합칠지는 별건으로 잰다 -
   Buffer 댓글수가 비로그인 경로보다 2~3 낮아(대댓글 미포함 추정) 값이 조용히
   바뀐다. 계획: _docs/20260909_02 (T7, §4.2)
"""
