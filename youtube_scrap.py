...
#
# 마커 이름 하나만 지우면 다음 마커에 또 뚫리므로 주석 형태를 지운다. 다만 범위를
# 한 줄 · 200자로 묶는다. `<!--.*?-->` 를 DOTALL 로 걸었더니 요약이 화살표 표기
# (`A --> B`)를 쓸 때 여는 주석과 짝이 맞아 그 사이 본문이 통째로 사라졌다
# (실측: 85자 요약에서 38자 소실). 실제 마커는 한 줄짜리라 이 제한으로 충분하다.
