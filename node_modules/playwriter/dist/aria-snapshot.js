...
const sharpPromise = import('sharp')
    ...
    .catch(() => { return null; });
export const DEFAULT_SNAPSHOT_FORMAT = 'interactive-dedup';
/**
 ...
 */
export function formatSnapshot(snapshot, format, logger) {
    if (format === 'raw') {
        return snapshot;
    }
    try {
 ...
 */
export function compactSnapshot(snapshot, options = {}) {
    const { removeCursorPointer = true, removeActive = true, removeEmptyStructural = true, removeTextSeparators = true, removeUrls = false, } = options;
 ...
 */
export function interactiveSnapshot(snapshot, options = {}) {
    const { keepUrls = false, keepImages = true, keepStructure = true, keepHeadings = true, removeGenericWrappers = true, } = options;
...
function extractInteractiveFlat(lines, interactiveRoles, keepUrls) {
    const result = [];
...
function extractInteractiveWithStructure(lines, interactiveRoles, contentRoles, keepUrls) {
...
function collapseGenericWrappers(snapshot) {
    const lines = snapshot.split('\n');
    const result = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
 ...
 */
export function deduplicateSnapshot(snapshot) {
    const lines = snapshot.split('\n');
    const nodes = [];
    const stack = [];
...
function collectDescendantNames(node) {
    const names = new Set();
...
function isNameRedundant(name, descendantNames) {
...
function rebuildLines(node, result) {
...
export async function getAriaSnapshot({ page, refFilter }) {
    ...
    const fetchRefHandle = async (ref) => {
        try {
            const locator = page.locator(`aria-ref=${ref}`);
            const handle = await locator.elementHandle({ timeout: 1000 });
            if (!handle) {
    ...
    const addRefInfo = ({ ref, handle, info }) => {
    ...
    const probeRefsFromSnapshot = async (refs) => {
...
function extractRefInfoFromSnapshot(snapshot) {
    const lines = snapshot.split('\n');
